    ...
    makeReadableWebSocketStream(webSocket, request.headers.get('sec-websocket-protocol') || '', log)
        .pipeTo(new WritableStream({
            async write(chunk) {
                ...
                // 处理新连接
                const {
                    hasError, message, addressType, portRemote = 443,
                    addressRemote = '', rawDataIndex,
                    维列斯Version = new Uint8Array([0, 0]), isUDP
                ...
                // UDP处理
                if (isUDP) {
                    if (portRemote !== 53) throw new Error('UDP 代理仅对 DNS（53 端口）启用');
                    isDns = true;
                }
                ...
                handleTCPOutBound(remoteSocketWapper, addressType, addressRemote, portRemote,
                    rawClientData, webSocket, 维列斯ResponseHeader, log);
            },
...
async function getEditorHTML(FileName, hasKV, content, cmad) {
    try {
        const response = await fetch('https://raw.githubusercontent.com/jmcgillcode/el/refs/heads/main/editor.html');
...
async function KV(request, env, txt = 'ADD.txt') {
    try {
        ...
        const html = await getEditorHTML(FileName, hasKV, content, cmad);
